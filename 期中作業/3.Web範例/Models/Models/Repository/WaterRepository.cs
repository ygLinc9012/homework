...
namespace YingG.Repository
{
    public class WaterRepository
    {
        ...
        public List<Station> GetProduct()
        {
            ...
            comm.CommandText = "select * from Stations";

            System.Data.SqlClient.SqlDataReader reader = comm.ExecuteReader();
            while (reader.Read())
            {
                YingG.Model.Station item = new YingG.Model.Station();
                item.LocationAddress= reader["LocationAddress"].ToString();
                item.ObservatoryName= reader["ObservatoryName"].ToString();
                item.ElevationOfWaterLevelZeroPoint= reader["ElevationOfWaterLevelZeroPoint"].ToString();
...
station.LocationAddress, station.ObservatoryName, station.ElevationOfWaterLevelZeroPoint);
        ...
        public void DeleteProduct(int id)
        {
            ...
            comm.CommandText = string.Format(@"Delete From Stations where ID='{0}'", id);
